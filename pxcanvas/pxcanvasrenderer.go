...
func (renderer *PxCanvasRenderer) Objects() []fyne.CanvasObject {
	// 5 iterations for 4 borders and the image itself
...
func (renderer *PxCanvasRenderer) Layout(size fyne.Size) {
	// make sure to call `LayoutCanvas` first as it resizes the img
...
func (renderer *PxCanvasRenderer) LayoutCanvas(fyne.Size) {
	imgPxWidth := renderer.pxCanvas.PxCols
	imgPxHeight := renderer.pxCanvas.PxRows
	pxSize := renderer.pxCanvas.PxSize
	renderer.canvasImage.Move(fyne.NewPos(renderer.pxCanvas.CanvasOffset.X,
...
func (renderer *PxCanvasRenderer) LayoutBorder(fyne.Size) {
